...
class PacienteForm(tk.LabelFrame):
    ...
    def mostrarEstadoPacienteEnFormulario(self,paciente):
        values=(paciente.getApellido(),paciente.getNombre(),paciente.getTelefono(),
...
class ImcPaciente(tk.Toplevel):
    ...
    def __init__(self,parent):
        ...
        self.imcEntry=tk.Entry(self, width=25)
        self.compCorp=tk.Label(self,text='Composicion Corporal')
        self.compCorpEntry=tk.Entry(self, width=25)
        ...
        self.compCorp.grid(column= 1,row=2,pady=10)
        self.compCorpEntry.grid(column= 2,row=2,pady=10)
        self.btn_salir.grid(column= 2, row=3,pady=10)

    def Imc(self,paciente):
        imc=paciente.getIMC()
        composicion=paciente.getCompCorporal(imc)
        cadena = '%.2f Kg/m2' % (imc)
        self.imcEntry.delete(0, tk.END)
        self.imcEntry.insert(0, cadena)
        self.compCorpEntry.delete(0, tk.END)
        self.compCorpEntry.insert(0, composicion)
